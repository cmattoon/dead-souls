...
external_port_1 : telnet {{ .serverPort }}

# pathname of mudlib
mudlib directory : {{ .mudLibDir }}

# pathname of driver/config dir
binary directory : {{ .mudBinDir }}
...
address server ip : {{ .addrServerName }}
address server port : {{ .addrServerPort }}
...
log directory : {{ .mudLogDir }}

# the directories which are searched by #include <...>
# for multiple dirs, separate each path with a ':'
include directories : {{ .secureDir }}/include:/include

# Directory to save binaries in.  (if BINARIES is defined)
save binaries directory : {{ .secureDir }}/save/binaries

# the file which defines the master object
master file : {{ .secureDir }}/daemon/master

# the file where all global simulated efuns are defined.
simulated efun file : {{ .secureDir }}/sefun/sefun

# file to swap out objects; not used if time to swap is 0
swap file : {{ .secureDir }}/tmp/swap
...
default fail message : {{ .defaultFailMessage }}
